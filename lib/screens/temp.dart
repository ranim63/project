...
// Define a StatefulWidget to handle temperature data visualization.
...
class _TemperatureScreenState extends State<TemperatureScreen> {
  ...
  Future<void> _saveData() async {
    ...
    final List<String> dataStrings = _data
        ...
        .toList();
    await prefs.setStringList('temperature_data', dataStrings);
  }

  // Method to load saved data from local storage.
  Future<void> _loadData() async {
    final prefs = await SharedPreferences.getInstance();
    final List<String>? dataStrings = prefs.getStringList('temperature_data');
    if (dataStrings != null) {
      setState(() {
        _data = dataStrings
            .map((e) {
  ...
  // Build method to define the UI of the TemperatureScreen.
  @override
